...
fastify.get(
  ...
  async (request, reply) => {
    ...
    try {
      ...
      const formattedData = formatOrdersData(orders);
      reply.send({ orders: formattedData });
    } catch (error) {
      request.log.error(error);
      reply.status(500).send({ error: "Failed to fetch total orders" });
    } finally {
...
fastify.get(
  ...
  async (request, reply) => {
    ...
    try {
      const orders = await fetchAllOrders(email, n);
      const formattedData = formatOrdersData(orders);
      const formattedOrder = formattedData.find(
        (order) => order.id == `gid://shopify/Order/${request.params.id}`
      );
      // const formattedData = formatOrdersData(order)
      reply.send({ order: formattedOrder });
    } catch (error) {
      request.log.error(error);
      reply.status(500).send({ error: "Failed to fetch total orders" });
    } finally {
      const duration = Date.now() - start;
      console.log(`GET /order/${id} processed in ${duration}ms`);
    }
  }
);
    ...
    try {
      ...
      const formattedOrders = orders.data.orders.edges.map((order) => {
        ...
        const newOrder = {
          ...
          financialStatus:
            node.displayFinancialStatus.slice(0, 1).toUpperCase() +
            node.displayFinancialStatus.slice(1).toLowerCase(),
        };
        return newOrder;
      });
...
fastify.get(
  ...
  async (request, reply) => {
    ...
    try {
      ...
      reply.send({ user: rest });
    } catch (error) {
      console.error("Error fetching user:", error);
      reply.status(500).send({ error: "Failed to fetch user" });
    } finally {
      const duration = Date.now() - start;
      console.log(`POST /user/:id processed in ${duration}ms`);
    }
  }
);
...
fastify.get(
  ...
  async (request, reply) => {
    ...
    try {
      ...
      console.log("Articles: Database error ", error);
      reply.status(500).send({ error: "Database error" });
    } finally {
      const duration = Date.now() - start;
...
fastify.post("/is-email-taken", async (request, reply) => {
  const start = Date.now();
  try {
    ...
    if (user) {
      reply.send({ emailTaken: true });
    } else {
      reply.send({ emailTaken: false });
    }
  } catch (error) {
    console.log("Error: ", error);
    request.log.error(error);
    ...
    const duration = Date.now() - start;
    console.log(`POST /is-email-taken processed in ${duration}ms`);
  }
});
...
fastify.post("/send-otp", async (request, reply) => {
  ...
  console.log("Send OTP request body:", request.body);
...
`,
    ...
    if (r.error) {
      console.error("Failed to send OTP:", r);
...
fastify.post("/signup", async (request, reply) => {
  try {
    let { email, password, firstName, lastName, authMethod, phone } =
      request.body;
...
fastify.put(
  ...
  async (request, reply) => {
    ...
    try {
      ...
      reply.status(500).send({ error: "Failed to update name" });
    }
  }
);
...
fastify.put(
  ...
  async (request, reply) => {
    let { newEmail, otp } = request.body;
    const { userId } = request.user;
    const currentEmail = request.user.email.toLowerCase();
    ...
    try {
      ...
      if (newEmail === currentEmail) {
        return reply
          .status(400)
          .send({ error: "New email cannot be the same as current email" });
      }
      ...
      if (!storedOtp) {
        return reply
          .status(400)
          .send({ error: "OTP not found or already used" });
      }
      ...
      const duration = Date.now() - start;
      console.log(`PUT /update-email processed in ${duration}ms`);
    }
  }
);
...
fastify.put(
  ...
  async (request, reply) => {
    ...
    try {
      ...
      reply.status(500).send({ error: "Failed to update phone number" });
    }
  }
);
